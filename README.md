...
| コマンド | 説明 |
| -------- | ---- |
| `/watch add account:<X アカウント名かプロフィール URL> [channel:<チャンネル>] [posts/quotes/reposts/replies:<true か false>] [media:<すべて か 画像付きのみ>]` | 監視対象を追加し、投稿先チャンネルを紐づける。アカウント名は登録済みのものが候補に出るが自由入力もできる。チャンネル省略時は実行したチャンネル。登録済みの組に再実行すると、指定した項目だけを更新する |
...
- `/watch add` の `account` にはアカウント名のほか、プロフィールや個別投稿の URL をそのまま貼れる (`https://x.com/<アカウント名>/media?filter=photo` のようにタブやクエリが付いていてもよい)。`/watch remove` と補完はアカウント名だけを受け付ける。
...
- `media:画像付きのみ` を選ぶと、その経路には画像が添付された投稿だけを送る。種別の設定とは AND で効くので、返信を切って画像付きのみにすれば、画像付きの通常投稿・引用・リポストだけが届く。対象は X に直接添付された画像で、動画、GIF、リンクのカード画像、引用元に付いた画像は含めない。リポストは元投稿の添付を、引用は引用者自身の添付を見る。
- Web Push の通知にはメディアの情報が無いため、画像付きのみの経路がある監視対象では投稿 1 件を内部 GraphQL で引いて判定する。引けなかった投稿は送らない。多くは内部 GraphQL のポーリングが同じ投稿を拾い直して送るが、後続の取得に現れなければ届かない。
